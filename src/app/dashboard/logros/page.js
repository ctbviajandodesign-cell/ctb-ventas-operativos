    ...
    <div className="space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs font-black text-gray-400 uppercase tracking-[0.25em] ml-1">
            Tu rendimiento personal — {profile?.nombre}
          </p>
        </div>
        {myRank > 0 && (
          <div className="bg-gray-900 text-white px-6 py-3 rounded-2xl text-center shadow-xl">
            <p className="text-xs font-black text-primary uppercase tracking-widest">Tu Posición</p>
            <p className="text-3xl font-black">{myRank === 1 ? '🥇' : myRank === 2 ? '🥈' : myRank === 3 ? '🥉' : `#${myRank}`}</p>
            <p className="text-xs text-gray-400 uppercase font-bold">de {allOps.length} asesores</p>
          </div>
        )}
      ...
      <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-gray-50">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Meta del Mes Actual</p>
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-black text-gray-900">${stats.ganancia.toLocaleString()}</span>
              <span className="text-gray-400 font-bold text-base">de ${stats.meta.toLocaleString()}</span>
            </div>
          </div>
          <div className={`text-right`}>
            ...
            <p className="text-xs font-bold text-gray-400 uppercase mt-1">Cumplido</p>
          </div>
        </div>
        ...
        <div className="flex justify-between mt-3">
          <span className="text-xs text-gray-400 font-bold uppercase">Faltan: ${Math.max(0, stats.meta - stats.ganancia).toLocaleString()}</span>
          {stats.cumplimiento >= 100 && <span className="text-xs text-success font-black uppercase">🎉 ¡Meta cumplida!</span>}
        </div>
      </div>
        ...
        {[
          ...
          <div key={kpi.label} className={`p-6 rounded-[2rem] border shadow-sm ${kpi.color}`}>
            <div className="flex items-center gap-2 mb-3">
              <kpi.icon size={16} className={kpi.darkMode ? 'text-primary' : kpi.text} />
              <p className={`text-xs font-black uppercase tracking-widest ${kpi.darkMode ? 'text-primary' : 'text-gray-400'}`}>{kpi.label}</p>
            </div>
            <p className={`text-3xl font-black ${kpi.darkMode ? 'text-white' : kpi.text}`}>{kpi.val}</p>
            <p className={`text-xs mt-1 font-bold ${kpi.darkMode ? 'text-gray-400' : 'text-gray-400'}`}>{kpi.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-gray-50">
          <h3 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-6 flex items-center gap-2">
            <BarChart3 size={16} className="text-primary" /> Mis Cotizaciones por Estado
          </h3>
          {totalCots === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 opacity-30">
              <AlertCircle size={40} />
              <p className="text-sm font-black uppercase mt-3">Sin cotizaciones registradas</p>
            </div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                {[
                  ...
                  <div key={item.label} className={`flex items-center justify-between p-3.5 rounded-2xl ${item.color}`}>
                    <div>
                      <p className="text-xs font-black uppercase opacity-90">{item.label}</p>
          ...
          <div className="h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyData} barSize={28}>
                ...
                <XAxis dataKey="mes" axisLine={false} tickLine={false} tick={{ fill: '#64748B', fontSize: 12, fontWeight: 900 }} />
                ...
                <Tooltip
                  ...
                  content={({ active, payload }) => {
                    if (active && payload?.length) {
                      return (
                        <div className="bg-gray-900 text-white p-4 rounded-2xl shadow-2xl">
                          <p className="text-xs font-black text-primary uppercase tracking-widest">{payload[0].payload.mes}</p>
                          <p className="text-xl font-black">${payload[0].value.toLocaleString()}</p>
                          <p className="text-xs text-gray-400 mt-1">{payload[0].payload.cerradas} cotizaciones ganadas</p>
                        </div>
                      )
                    }
                    return null
      ...
      <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-gray-50">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-sm font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
            <Trophy size={16} className="text-amber-500" /> Ranking Completo del Equipo — Mes Actual
          </h3>
          <span className="text-xs font-black text-primary bg-primary/10 px-3 py-1.5 rounded-full uppercase tracking-widest">
            {allOps.length} asesores
          </span>
        </div>
        {allOps.length === 0 ? (
          <p className="text-base text-gray-400 text-center py-8">Sin datos disponibles</p>
        ) : (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center font-black text-base bg-gray-200 text-gray-600">
                        {op.avatar}
                      </div>
                      <div>
                        <p className={`font-black text-base ${op.isMe ? 'text-primary' : 'text-gray-800'}`}>
                          {op.nombreCompleto}
                          {op.isMe && <span className="ml-2 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full uppercase font-black">Tú</span>}
                        </p>
                        <p className="text-xs text-gray-400 font-bold uppercase mt-0.5">Meta: ${op.meta.toLocaleString()}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-black text-xl text-gray-900">${op.ganancia.toLocaleString()}</p>
                      <p className={`text-xs font-black uppercase ${op.cumplimiento >= 100 ? 'text-success' : op.cumplimiento >= 60 ? 'text-primary' : 'text-amber-600'}`}>

...
export default function DashboardPage() {
  ...
  const [metrics, setMetrics] = useState({
    ...
    pipeline: 0,
    topDestino: 'N/A',
    globalGoal: 50000 // Valor por defecto o dinámico
  })
  ...
  async function fetchDashboardData() {
    try {
      ...
      } else if (selectedOperative !== 'global') {
        ventasQuery = ventasQuery.eq('operativo_id', selectedOperative)
      }
      ...
      if (!isAdmin) {
        pipelineQuery = pipelineQuery.eq('operativo_id', user.id)
      } else if (selectedOperative !== 'global') {
        pipelineQuery = pipelineQuery.eq('operativo_id', selectedOperative)
      }
      ...
      // 3. Leaderboard y Gráficos (Solo Admin)
      if (isAdmin) {
      ...
      if (!isAdmin) {
        ...
        quotesQuery = quotesQuery.eq('operativo_id', selectedOperative)
      }
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                  ...
                  <BarChart data={chartData}>
                    ...
                    <Bar dataKey="total" radius={[10, 10, 10, 10]} barSize={40}>
                      {chartData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={index === 0 ? '#0066CC' : '#E2E8F0'} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
        ...
        <div className="space-y-10">
          ...
          {/* CUMPLIMIENTO DE META (PERSONAL O GLOBAL) */}
          <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-gray-50 relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-10">
              ...
              <div className="bg-primary/10 px-3 py-1 rounded-full">
                <span className="text-[10px] font-black text-primary uppercase">Mes Actual</span>
              </div>
            </div>
            
            <div className="flex items-end justify-between mb-4">
              <div>
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-2">Completado</p>
                <p className="text-4xl font-black text-gray-900 tracking-tighter">${metrics.metaComputable.toLocaleString()}</p>
              </div>
              <div className="text-right">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-2">Objetivo</p>
                <p className="text-xl font-black text-gray-400">${metrics.globalGoal.toLocaleString()}</p>
              </div>
            </div>

            <div className="w-full bg-gray-100 h-8 rounded-full overflow-hidden mb-6 p-1.5 border border-gray-50">
              <div 
                className="h-full bg-gradient-to-r from-primary to-blue-400 rounded-full transition-all duration-1000 ease-out shadow-[0_0_20px_rgba(0,102,204,0.3)] relative overflow-hidden"
            ...
            <div className="flex items-center justify-between bg-gray-50 p-4 rounded-2xl">
              <div className="flex items-center gap-2 text-xs font-black text-gray-800">
                ...
                {metrics.porcentajeMeta.toFixed(1)}% Cumplido
              </div>
              <div className="text-[9px] font-bold text-gray-400 uppercase">
                Restan ${(metrics.globalGoal - metrics.metaComputable).toLocaleString()}
              </div>
            </div>
          </div>
              ...
              <div className="space-y-6">
                {leaderboard.length > 0 ? leaderboard.map((op, idx) => (
                  <div key={op.id} className="flex items-center justify-between group cursor-pointer" onClick={() => setSelectedOperative(op.id)}>
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg transition-all ${idx === 0 ? 'bg-amber-100 text-amber-600 shadow-lg shadow-amber-200' : 'bg-gray-50 text-gray-400'}`}>
                        {op.avatar}
                      </div>
                      <div>
                        <p className="text-sm font-black text-gray-900 leading-none mb-1 group-hover:text-primary transition-colors">{op.nombre}</p>
                        <div className="flex items-center gap-2">
                          <div className="w-20 bg-gray-100 h-1.5 rounded-full overflow-hidden">
                            <div className="h-full bg-success rounded-full" style={{ width: `${Math.min(op.cumplimiento, 100)}%` }}></div>
                          </div>
                  ...
                  <p className="text-xs text-gray-400 italic text-center">Sin actividad en el periodo</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="bg-gray-900 p-10 rounded-[4rem] text-white shadow-2xl relative overflow-hidden">
            ...
            <h3 className="font-black text-2xl uppercase tracking-tighter mb-6 relative z-10 flex items-center gap-3">
            ...
            <div className="space-y-8 relative z-10">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/5 p-4 rounded-3xl border border-white/5">
                  <p className="text-[9px] font-black text-primary uppercase tracking-widest mb-1">Top Destino</p>
                  <p className="text-lg font-black uppercase italic leading-tight truncate">{metrics.topDestino}</p>
                </div>
                <div className="bg-white/5 p-4 rounded-3xl border border-white/5">
                  <p className="text-[9px] font-black text-success uppercase tracking-widest mb-1">Conversión</p>
              ...
              <div className="bg-primary/10 p-6 rounded-[2rem] border border-primary/20">
                <div className="flex items-start gap-3">
                  <div className="bg-primary p-2 rounded-xl text-white mt-1">
                    <Target size={16} />
                  </div>
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest mb-1">Estrategia sugerida</p>
                    <p className="text-xs italic leading-relaxed text-gray-300">
                      "El pipeline actual de **${metrics.pipeline.toLocaleString()}** indica una oportunidad de cierre agresivo. {selectedOperative === 'global' ? 'Asigna seguimientos prioritarios al Top Performers.' : 'Enfócate en las cotizaciones de mayor valor para llegar a tu meta.'}"
                    </p>
                  </div>
                </div>
              </div>

        ...
        <div className="px-6 py-6 border-b border-gray-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[8px] text-primary font-black uppercase tracking-[0.2em] mt-0.5">
          ...
          {profile?.nombre && (
            <div className="mt-4 bg-gray-50 rounded-2xl px-3 py-2 flex items-center gap-2">
              <div className="w-7 h-7 bg-primary rounded-xl flex items-center justify-center text-white text-[10px] font-black">
                {profile.nombre.charAt(0).toUpperCase()}
              </div>
              <div>
                <p className="text-[10px] font-black text-gray-800 leading-none">{profile.nombre.split(' ')[0]}</p>
                <p className="text-[8px] text-gray-400 uppercase tracking-widest mt-0.5">{profile.rol}</p>
              </div>
            </div>
          )}
        </div>
          ...
          {profile?.rol === 'admin' && (
            <>
              <div className="pt-3 pb-1">
                <p className="text-[8px] font-black text-gray-300 uppercase tracking-[0.25em] px-4">Administración</p>
              </div>
              <Link
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <header className="h-14 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-8 sticky top-0 z-10">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-black text-gray-400 uppercase text-[9px] tracking-[0.3em]">
              CTB Intelligence · {profile?.nombre || 'Cargando...'}
            </h2>
          </div>
          <div className="flex items-center gap-3">
            <div className="bg-success/10 px-3 py-1 rounded-full">
              <p className="text-[9px] font-black text-success uppercase tracking-widest">Sistema activo</p>
            </div>
          </div>
        </header>

...
export async function POST(request) {
  try {
    ...
    if (modo === 'GLOBAL_ADMIN') {
      prompt = `Eres un Director Comercial de turismo experto. Analiza este resumen global del equipo de ventas y da exactamente 2 oraciones de consejo estratégico en español simple, sin jerga, para motivar al equipo y alcanzar la meta global. Sé directo y enfocado en números finales.
...
Responde SOLO con el consejo en 2 oraciones. Sin títulos ni bullets.`
      ...
      prompt = `Eres un Mentor de Ventas experto analizando a un asesor específico para darle feedback al Administrador. Da exactamente 2 oraciones de evaluación objetiva en español simple sobre el rendimiento de este asesor y qué aspecto clave debe ajustar.
...
- Vouchers Emitidos: ${metricas.vouchers || 0}
...
Responde SOLO con la evaluación en 2 oraciones. Sin títulos ni bullets.`
      ...
      prompt = `Eres un Coach de Ventas de turismo motivador y directo. Estás hablando directamente con el asesor de viajes sobre su desempeño actual. Da exactamente 2 oraciones de consejo práctico en español simple, sin jerga, que le ayuden a mejorar sus comisiones, gestionar sus proformas inactivas o perdidas y agilizar sus cierres.
